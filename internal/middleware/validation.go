...
func ValidationMiddleware(validationSt any) func(*gin.Context) {

	return func(ctx *gin.Context) {
		content := ctx.GetHeader("Content-Type")

		st,err := switchHeader(ctx, validationSt)
		if ok := exception.ServerException(ctx, err); ok {
			ctx.Abort()
			return
		}

		err = validation.ValidReq.Struct(st.Interface())
		...
		if strings.Contains(content, "multipart/form-data") {
			...
			err := validation.ValidationReqFile(st.Addr().Interface(), "file")
			if ok := exception.BadReqException(ctx, err); ok {
				ctx.Abort()
				return
			}
