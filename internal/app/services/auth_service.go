...
func (a *AuthService) Login(ctx *gin.Context, field validation.Login) (table.Accounts, string, error) {
	account, err := a.RepoAccount.GetUserByFields(ctx, map[string]any{"email": field.Email, "password": helpers.PasswordHashing(field.Password)})
	if err != nil {
		return account, "", err
	}

	token, err := helpers.CreateJWT(map[string]any{"email": account.Email, "id": account.ID})
	return account, token, err
}

func (a *AuthService) Register(ctx *gin.Context, field validation.Register) (string, error) {
	account := table.Accounts{Email: field.Email, FullName: field.FullName, Mobile: field.Mobile, Password: helpers.PasswordHashing(field.Password)}
